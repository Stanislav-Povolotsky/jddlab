...
python3 get-release-info.py https://github.com/Storyyeller/enjarify -fan '^src.zip$' -o $pkg_info
...
mv /tmp/extracted/enjarify*/* $pkg_path
rm -rf $fname /tmp/extracted/
popd

pushd $pkg_path
for filename in enjarify.sh; do
  cmd="${filename%.sh}"
