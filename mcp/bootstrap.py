#!/usr/bin/env python3
"""Bootstrap the jddlab MCP bundle from GitHub releases.

This file is intentionally dependency-free because installed `jddlab` launcher
scripts may not have the repository checkout next to them.
"""
...
def request_bytes(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "jddlab-mcp-bootstrap"})
...
def current_installer() -> Path:
    return current_dir() / "mcp" / "install.py"


def bundle_is_installed() -> bool:
...
def verify_sha256(path: Path, sha_url: str | None) -> None:
    if not sha_url:
        print("Warning: no .sha256 asset found for MCP bundle; skipping checksum verification.")
        return
...
def copy_bundle_to_install_root(bundle_zip: Path, version: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="jddlab-mcp-") as temp_name:
        temp_dir = Path(temp_name)
        with zipfile.ZipFile(bundle_zip) as zf:
            zf.extractall(temp_dir)
        candidates = [temp_dir / "jddlab-mcp", *[p for p in temp_dir.iterdir() if p.is_dir()]]
        bundle_root = next((p for p in candidates if (p / "mcp" / "server.py").exists()), None)
        if not bundle_root:
            raise RuntimeError("Downloaded MCP bundle does not contain mcp/server.py")
...
def install_or_update(force: bool = False) -> Path:
    if bundle_is_installed() and not force:
        return current_dir()
    ...
    print(f"Installed jddlab MCP bundle to {installed}")
    return installed
...
def main(argv: list[str] | None = None) -> int:
    ...
    if sys.version_info < (3, 10):
        print("Python 3.10 or newer is required to run the jddlab MCP server.", file=sys.stderr)
        return 1
    ...
    if not installer.exists():
        print(f"MCP installer was not found: {installer}", file=sys.stderr)
        return 1
