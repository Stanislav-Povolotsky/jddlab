...
Sign APK files with dex2jar signing helpers. This is the explicit `d2j-` command name.
...
```text
apk-sign [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `apk-sign` and `d2j-apk-sign` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-apk-sign <args>
```
