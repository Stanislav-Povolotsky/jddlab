...
Weave or transform Java JAR/class files. This is the explicit `d2j-` command name.
...
```text
jar-weaver [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `jar-weaver` and `d2j-jar-weaver` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-jar-weaver <args>
```
