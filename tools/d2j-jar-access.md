...
Inspect or rewrite Java class/JAR access flags. This is the explicit `d2j-` command name.
...
```text
jar-access [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `jar-access` and `d2j-jar-access` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-jar-access <args>
```
