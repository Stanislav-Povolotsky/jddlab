...
Convert Java JAR/class files to DEX. This is the explicit `d2j-` command name.
...
```text
jar2dex [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `jar2dex` and `d2j-jar2dex` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-jar2dex <args>
```
