...
Run `simplify -h` for the complete option list. Typical arguments include input APK/DEX/smali targets, output locations, class or method filters, execution limits, and verbosity flags.

## Example

```json
{"args":["classes.dex","-o","simplified.dex"],"input_paths":["classes.dex"],"output_paths":["simplified.dex"]}
```
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest simplify <args>
```
