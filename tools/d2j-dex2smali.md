...
Convert DEX files to smali text. This is the explicit `d2j-` command name.
...
```text
dex2smali [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `dex2smali` and `d2j-dex2smali` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-dex2smali <args>
```
