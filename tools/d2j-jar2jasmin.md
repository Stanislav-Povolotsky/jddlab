...
Convert JAR/class files to Jasmin assembly. This is the explicit `d2j-` command name.
...
```text
jar2jasmin [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `jar2jasmin` and `d2j-jar2jasmin` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-jar2jasmin <args>
```
