...
Run dex2jar string decryption helper. This is the explicit `d2j-` command name.
...
```text
decrypt-string [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `decrypt-string` and `d2j-decrypt-string` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-decrypt-string <args>
```
