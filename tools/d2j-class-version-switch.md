...
Change Java class file version values. This is the explicit `d2j-` command name.
...
```text
class-version-switch [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `class-version-switch` and `d2j-class-version-switch` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-class-version-switch <args>
```
