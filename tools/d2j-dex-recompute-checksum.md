...
Recompute DEX checksums. This is the explicit `d2j-` command name.
...
```text
dex-recompute-checksum [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `dex-recompute-checksum` and `d2j-dex-recompute-checksum` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-dex-recompute-checksum <args>
```
