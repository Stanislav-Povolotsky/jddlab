...
Convert Android DEX or APK files to Java JAR files. This is the explicit `d2j-` command name.
...
The canonical command may also be available with a `d2j-` prefix. For example, `dex2jar` and `d2j-dex2jar` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-dex2jar <args>
```
