...
Command-line helper to decompile a native binary file with Ghidra.

## Usage

```text
ghidra-decompile <input-binary-file> [<output-file-for-c-code>]
```

## Example

```json
{"args":["protected.so","protected.c"],"input_paths":["protected.so"],"output_paths":["protected.c"]}
```
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest ghidra-decompile <args>
```
