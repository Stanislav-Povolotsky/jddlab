...
Assemble Jasmin sources into JAR/class files. This is the explicit `d2j-` command name.
...
```text
jasmin2jar [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `jasmin2jar` and `d2j-jasmin2jar` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-jasmin2jar <args>
```
