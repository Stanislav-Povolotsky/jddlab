...
Normalize or standardize APK files with dex2jar helpers. This is the explicit `d2j-` command name.
...
```text
std-apk [options] <file0> [file1 ... fileN]
```

The canonical command may also be available with a `d2j-` prefix. For example, `std-apk` and `d2j-std-apk` are aliases when both exist in the image.
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest d2j-std-apk <args>
```
