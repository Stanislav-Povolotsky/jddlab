...
Assembler for Android DEX bytecode.

## Usage

```text
smali [-h] [-v] [<command [<args>]]
```

## Main modes

- `smali assemble`, `smali ass`, `smali as`, `smali a`: assemble smali files into a dex file.
- `smali help <command>`: show command-specific help.
...
```json
{"args":["assemble","smali","-o","classes.dex"],"input_paths":["smali"],"output_paths":["classes.dex"]}
```
...
```text
docker run --rm -v "<host-mount-root>:/work" stanislavpovolotsky/jddlab:latest smali <args>
```
